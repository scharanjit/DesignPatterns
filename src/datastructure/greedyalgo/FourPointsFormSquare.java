...
1.)There are two points, say A and C which are distance x from each other,
 and two other points, say B and D which are also distance x from each other.

2.)Each point {A, B, C, D} is an equal distance from the two points
which aren't x away. i.e.: If A is x away from C, then it will be z away
 from both B and D.
