 ...
 * <p>
 *     HashMap contains key as key and value as Node of doubly linked list
 * <p>
...
public class LRUCache {
    ...
    public int get(int key) {
        if (hashMap.containsKey(key)) {
            ...
            remove(n);   //remove it after checking it is not head or end
            setHead(n); ///make is most
            return n.value;
    ...
    public void remove(Node node) {
        //nee to check whether the node to be deleted is
        // end or head or in between
        //since it is doubly linked list

        if (node.pre != null) {
            node.pre.next = node.next;
        } else {
            head = node.next;  //if node to be deleted is head,point head to next
        }
